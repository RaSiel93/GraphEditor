...
public class Graph {
    ...
    public void setVertexes(ArrayList<Vertex> temp_vertexes) {
	vertexes = temp_vertexes;
    }

    public void setEdges(ArrayList<Edge> temp_edges) {
	edges = temp_edges;
    }
	...
	for (int numVertex = 0; numVertex < countVertex(); numVertex++) {
	    if (getVertex(numVertex).getX() > maxCoordX) {
		maxCoordX = getVertex(numVertex).getX();
	    }
	    if (getVertex(numVertex).getY() > maxCoordY) {
		maxCoordY = getVertex(numVertex).getY();
	    }
	}
	return new Dimension(((int) maxCoordX + 100), ((int) maxCoordY) + 100);
    }
	...
	for (int numEdge = 0; numEdge < countEdge(); numEdge++) {
	    if (getEdge(numEdge).getV1() == v1
		    && getEdge(numEdge).getV2() == v2) {
		return true;
	    }
	}
	return false;
	...
	for (int numEdge = 0; numEdge < countEdge(); numEdge++) {
	    if (getEdge(numEdge).getV1() == v2
		    && getEdge(numEdge).getV2() == v1) {
		return numEdge;
	    }
	}
	return -1;
    ...
    // -------------------------------------
    private static List<JLabel> namesVertexes;
